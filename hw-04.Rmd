...
a.  Use the R function rpois() to generate 10,000 $x_i$ from a regular Poisson distribution, $X \sim \textrm{Poisson}(\lambda=1.5)$. Plot a histogram of this distribution and note its mean and variance. Next, let $Y \sim \textrm{Gamma}(r = 3, \lambda = 2)$ and use rgamma() to generate 10,000 random $y_i$ from this distribution.
    ...
    r <- 3
    lambda_gamma <- 2
    gamma_data <- rgamma(n, shape = r, rate = 1 / lambda_gamma)
    ...
    When comparing the two histograms, we can clearly see that the most common value from the Poisson distribution is 1, where for the gamma-Poisson, it is 3. The other major difference is the length of the right tail. Both distributions are right-skewed, but the gamma-Poisson has a much larger right tail, extending all the way to 32, where the Poisson only extends to 8. We can see these differences clearly through descriptive statistics with the gamma-Poisson having a larger mean and variance (mean = 6, variance = 18) than the Poisson (mean = 1.5, variance = 1.5). It should also be noted that the mean and variance are (approximately) equal for the Poisson but not for the gamma-Poisson.
...
b.  A negative binomial distribution can actually be expressed as a gamma-Poisson mixture. In Part a, you looked at a gamma-Poisson mixture $Z \sim \textrm{Poisson}(\lambda)$ where $\lambda \sim \textrm{Gamma}(r = 3, \lambda' = 2)$.
    ...
    We also know the the variance should be the same, so we will set the variance of a negative binomial distribution equal to the variance of our gamma-Poisson mixture, which was 18:

    $$
    \frac{r(1 - p)}{p^2} = 18
    $$

    Now, we can substitute our solution for r(1-p) into this new equation and solve for r:
    ...
    Plugging this value back into the equation with r, we find that

    $$
    r\left(1 - \frac{1}{3}\right) = 6 \times \frac{1}{3}
    $$
    ...
    Now, we have both of our parameters for the negative binomial distribution.
    ...
    We can see that the histogram for this negative binomial distribution has approximately the same shape and median as the gamma-Poisson mixture. Also, the mean and variance are approximately equal for both distributions (mean = 6, variance = 18).

c.  Make an argument that if you want a $\textrm{NegBinom}(r, p)$ random variable, you can instead sample from a Poisson distribution, where the $\lambda$ values are themselves sampled from a gamma distribution with parameters $r$ and $\lambda' = \frac{1-p}{p}$. You may show equivalency via the simulations or mathematically, however, make sure your arguments are precise and clear.

    We have just conducted two simulations that outputted approximately equivalent results. In the first simulation, the gamma-Poisson distribution, we drew 10,000 times from this distribution modeled by $Z \sim \textrm{P oisson}(\lambda)$where $\lambda \sim \textrm{Gamma}(r = 3, \lambda' = 2)$. In the second simulation, we drew 10,000 times from a a distribution modeled by $\textrm{NegBinom}(3, \frac{1}{3})$. As we can see, the $\lambda'$ value (= 2) used in the gamma-Poisson distribution is equal to $\frac{1-p}{p}$ where $p = \frac{1}{3}$. We could see when comparing the histograms that the shape of the distributions are equal. The visualization below shows the two histograms overlapped to illustrate the equivalency.
