...
$$
\nabla^2_{\boldsymbol\beta} \log \mathcal{L}(p) = - \mathbf{X}^T \mathbf{W} \mathbf{X}
$$
...
$$
\boldsymbol\beta^{(t+1)} = \boldsymbol\beta^{(t)} - \left( \mathbf{X}^T \mathbf{W} \mathbf{X} \right)^{-1} \mathbf{X}^T \left( \mathbf{y} - \boldsymbol\pi \right)
$$
...
a\. Express a the following Poisson distribution in canonical form (as it is a part of the exponential family), and list the components:
...
The log-partition function $\psi(\lambda)$ is simply:

$$
\psi(\lambda) = \lambda
$$

This function normalizes the distribution, ensuring the total probability sums to 1.
...
$$
\mathbb{E}[X] = \frac{d}{d\eta(\lambda)} \psi(\lambda) = \frac{d}{d\eta(\lambda)} e^{\eta(\lambda)} = e^{\eta(\lambda)}$$
...
$$
\text{Var}(X) = \frac{d^2}{d\eta(\lambda)^2} \psi(\lambda) = \frac{d^2}{d\eta(\lambda)^2} e^{\eta(\lambda)} = e^{\eta(\lambda)}
$$
...
Correct part a. = 2 points

-   -1 for canonical form incorrect

-   -0.25 for each of the components that are incorrect

Correct part b. = 2 points

-   -1 for not using the log-partition function at all

-   -0.5 for not using the log-partition function correctly

-   -0.5 for incorrect mean calculation

-   -0.5 for incorrect variance calculation
