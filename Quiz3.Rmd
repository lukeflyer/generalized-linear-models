...
We know this is the case because we know that the first and second derivatives of $\psi(\lambda)$ are the mean and
variances, respectively, of the sufficient statistic $T(y)$. Since we know that $T(y) = y$, we can then say that the first and second derivatives of $\psi(\lambda)$ are the mean and variances, respectively, of $Y$. This is a convenient method of finding the mean when $T(y) = y$.
...
Now, differentiate the inner term $-\exp(-\exp(\mathbf{X}_i^\top \boldsymbol{\beta}))$ with respect to $\boldsymbol{\beta}$, and put it in numerator:
...
Noww, plug into the quotient rule:
...
\*Keep in mind the above equation is written on 2 lines, as it would not fit into 1.
